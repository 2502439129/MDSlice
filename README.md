...
> 本文档使用普通 markdown 语法编写，MDSlice版本见 [README-MDSlice.md](README-MDSlice.md)
...
| 类型 | 语法 |
| --- | --- |
| 折叠块 | `:::details 标题` … `:::`（`:::detail` 亦可）；标题写在标记同行，省略时显示「展开查看」 |
| 多列 | `:::cols` / `:::duo` 包住若干 `:::col 标题`（`:::column` 亦可）；标题可省，≤820px 自动变单列 |
| 标签页块 | `:::tabs` 包住若干 `:::tab 名称`（写 `:::col` 亦可）；名称省略时显示 `Tab` |
| 提示框（`:::` 写法） | `:::info` / `:::note` / `:::tip` / `:::warning` / `:::danger`（`:::caution` 同 warning），与 `> [!INFO]` 等价 |
| 徽章 | `[[必填]]`、`[[选填]]`，其他文字渲染成默认色徽章 |
| 元数据卡 | 章节标题下方紧跟 `---` 包裹的 `key: value` 块（如 `title`、`version`、`scope`） |
| 表格折叠树 | 表格首列行首写 `└`（`├` `│` `┌` `┬` `─` 亦可，每个计一层）表达层级，父行可折叠 |
| 表格列语义 | 表头出现 `字段` / `类型` / `必填` / `示例值` 等名字时按结构化表格判定（规则见 `MD结构说明.md` §4.1） |
| 围栏代码标题 | 围栏信息串里追加 `title="…"`（三个反引号 + 语言 + 该参数）→ 代码块顶部显示标题栏 |
| 文档内链接 | `[文字](#锚点)` 接管为章节内跳转（匹配顺序见 `MD结构说明.md` §3.2），不刷新页面 |

以上 `:::` 容器可互相嵌套，结束标记统一为单独一行的 `:::`。
